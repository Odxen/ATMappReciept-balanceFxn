...
# PRINT RECEIPT FUNCTION
    def rcptPrint(self,withdrawn):
        # CHECK IF USER WOULD LIKE TO PRINT A RECEIPT FOR THE TRANSACTION
        while self.atmui.receiptoptionBtn.clicked:
            self.atmui.main.setCurrentWidget(self.atmui.receiptoptionPage)
            
            # CHECK IF THE USER SAYS YES AND PRINT A RECEIPT FOR THE TRANSACTION i.e DISPLAY SPECIFICS OF THE TRANSACTION
            if self.atmui.confirmreceiptYesBtn.clicked:
                self.atmui.main.setCurrentWidget(self.atmui.receiptPage)
                self.atmui.ReceipttransactionnumberText.setText(str(random.randint(100000000000,999999999999)))
                self.atmui.ReceiptaccountnameText.setText(str(session['username']).upper())
                self.atmui.ReceiptamountwithdrawnText.setText(str(withdrawn[0])+" "+str(withdrawn[1]))      
                self.atmui.ReceiptnewbalanceText2.setText(str(withdrawn[2]))                 
                break
            
            # CHECK IF THE USER SAYS NO STOP THE LOOP
            elif self.atmui.confirmreceiptNoBtn.clicked: 
                break
    ...
    def crntBalance(self):    
        ...
        self.atmui.balanceText2.setText(currency +' '+ str(availableBalance))

        if self.atmui.backtoMenuBtn4Btn.clicked: 
            self.atmui.main.setCurrentWidget(self.atmui.newtransactionPage)
